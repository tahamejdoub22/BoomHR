...
async function getAllDeductions(req, res, next) {
  try {
    const deductions = await Deduction.find().populate('grossSalary_id', 'grossSalary') .populate({ path: 'grossSalary_id', populate: { path: 'employee_id', select: 'first_name last_name avatar' }});;
